...
import { TopupVerifyButtons } from "@/components/business/topup-verify-buttons";
...
export default async function BusinessPaymentsPage() {
  ...
  const [{ data: payments }, { data: topups }] = await Promise.all([
    ...
    supabase
      .from("wallet_topups")
      .select("id, amount, slip_path, created_at, profiles(full_name, phone)")
      .eq("status", "pending")
  ...
  return (
    <div className="flex flex-col gap-6 px-4 pt-4 pb-10">
      <div>
        <h1 className="text-xl font-bold text-ink-900">Order payments</h1>
        {(payments ?? []).length === 0 ? (
          <p className="mt-4 text-center text-ink-500">Nothing waiting on verification.</p>
        ) : (
          <div className="mt-3 flex flex-col gap-3">
            {(payments ?? []).map((payment: any) => (
              <div key={payment.id} className="flex flex-col gap-3 rounded-card bg-white p-4 shadow-card">
                ...
                <ViewSlipButton slipPath={payment.slip_path} bucket="payment-slips" />
      ...
      <div>
        <h2 className="text-xl font-bold text-ink-900">Wallet top-ups</h2>
        {(topups ?? []).length === 0 ? (
          <p className="mt-4 text-center text-ink-500">Nothing waiting on verification.</p>
        ) : (
          <div className="mt-3 flex flex-col gap-3">
            {(topups ?? []).map((topup: any) => (
              <div key={topup.id} className="flex flex-col gap-3 rounded-card bg-white p-4 shadow-card">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-semibold text-ink-900">{topup.profiles?.full_name}</p>
                    <p className="ltr-number text-sm text-ink-500">{topup.profiles?.phone}</p>
                  </div>
                  <p className="ltr-number font-semibold text-ocean-900">MVR {topup.amount.toFixed(2)}</p>
                </div>
                <ViewSlipButton slipPath={topup.slip_path} bucket="wallet-topup-slips" />
                <TopupVerifyButtons topupId={topup.id} />
              </div>
            ))}
          </div>
        )}
