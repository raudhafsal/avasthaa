...
import { addMenuCategory, addMenuItem } from "@/lib/actions/menu";
...
export function AddCategoryForm({ businessId }: { businessId: string }) {
  const [state, formAction] = useFormState(addMenuCategory, {});
...
export function AddItemForm({ businessId, categories }: { businessId: string; categories: Category[] }) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-3 rounded-card bg-white p-3 shadow-card">
      ...
      <TextField name="preparationTime" type="number" min="1" label="Prep time (minutes)" defaultValue={15} />
      {state.error && <p className="text-sm text-coral-600">{state.error}</p>}
      <div className="flex gap-2">
        <Button variant="ghost" type="button" onClick={() => setOpen(false)}>
          Cancel
